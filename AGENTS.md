...
Use this guide for automated work in the `surgeist-style` crate repo.
...
This repo owns the `surgeist-style` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It links this repo as a submodule under `crates/surgeist-style` and owns workspace wiring, submodule pointers, cross-crate plans, source-derived API coordination, and whole-system verification.
...
`surgeist-style` owns style model, resolution, and visual/layout property contracts.
...
1. Confirm the work belongs in `surgeist-style`.
